...
<div class="row g-3 mb-4">
    <div class="col-md-6 col-xl-3">
        <div class="stat-card stat-gold animate-slideUp stagger-1">
            <div class="stat-header"><div class="stat-icon"><i class="bi bi-clock-fill"></i></div></div>
    ...
    <div class="col-md-6 col-xl-3">
        <div class="stat-card stat-blue animate-slideUp stagger-2">
            <div class="stat-header"><div class="stat-icon"><i class="bi bi-eye-fill"></i></div></div>
    ...
    <div class="col-md-6 col-xl-3">
        <div class="stat-card stat-green animate-slideUp stagger-3">
            <div class="stat-header"><div class="stat-icon"><i class="bi bi-check-circle-fill"></i></div></div>
    ...
    <div class="col-md-6 col-xl-3">
        <div class="stat-card stat-red animate-slideUp stagger-4">
            <div class="stat-header"><div class="stat-icon"><i class="bi bi-calendar-event-fill"></i></div></div>
...
<!-- Scanner and Pending Violations Row -->
<div class="row g-3">
    <!-- QR Scanner Tool -->
    <div class="col-lg-4">
            ...
            <div class="panel-body text-center" style="padding: 15px;">
                ...
                <div id="resultSection" style="display:none; text-align: left;">
                    ...
                    <div class="d-grid gap-2">
                        <a href="#" id="historyBtn" class="btn btn-sm btn-outline-custom mt-2">
                            <i class="bi bi-clock-history"></i> View Full History
                        </a>
                        <a href="<?= BASE_PATH ?>/discipline/report_violation.php" class="btn-primary-custom mt-2 mb-1 d-inline-block text-center text-decoration-none" style="padding: 6px 14px; font-size: 13px; width: auto;">
                            <i class="bi bi-file-earmark-plus"></i> File Direct Report
                        </a>
                        <button class="btn btn-sm text-secondary" style="background:#f1f1f1;" onclick="resetScanner()">
    ...
    <!-- Pending Violations (Takes up remaining 8 cols) -->
    <div class="col-lg-8">
        <div class="card-panel mb-4">
    <div class="panel-header">
        <h5 class="panel-title"><i class="bi bi-exclamation-triangle text-warning"></i> Pending Violations</h5>
        <a href="<?= BASE_PATH ?>/discipline/violations.php?status=pending" class="btn btn-sm btn-outline-custom">View All</a>
    </div>
    <div class="data-table-wrapper">
        <table class="data-table">
            <thead><tr><th>Student</th><th>Violation</th><th>Severity</th><th>Reported By</th><th>Date</th><th>Actions</th></tr></thead>
            <tbody>
                ...
                <?php endforeach; ?>
                <?php if (empty($pending)): ?><tr><td colspan="6" class="text-center text-muted py-4">No pending violations</td></tr><?php endif; ?>
            </tbody>
        </table>
    </div>
</div>
...
<div class="card-panel">
    <div class="panel-header"><h5 class="panel-title"><i class="bi bi-hammer"></i> Recent Disciplinary Actions</h5></div>
            ...
            <div class="timeline-item <?= in_array($a['action_type'], ['suspension','expulsion']) ? 'danger' : ($a['action_type'] === 'warning' ? 'warning' : '') ?>">
                <div class="timeline-content">
                    ...
                    <p style="font-size:12px;margin:4px 0 0;color:var(--text-secondary);">
                        <?= sanitize(substr($a['description'] ?? '', 0, 100)) ?>
                    </p>
                </div>
    ...
    fetch("/api/qr_lookup.php?qr=" + encodeURIComponent(qrData))
        ...
        .then(data => {
            if (data.success) {
                ...
                document.getElementById("historyBtn").href = "/discipline/history.php?student=" + encodeURIComponent(s.student_number);
...
// Start scanner when page loads (only if camera is available)
